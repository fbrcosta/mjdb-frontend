...
function HospitalizationsRealPanel(props) {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      ...
      await axios.get(process.env.REACT_APP_BACKEND_URL + '/mjdb/api/v1/statistics/hospitalizationsreal')
        .then(response => {
          ...
          <Col xs={12} sm={6}>
            <Container>
              <Row>
                <Col xs={12} sm={6}>
                  <MetricBox 
                    metric={t('HospitalizationsRealPanel.average')} 
                ...
                <Col xs={12} sm={6}>
                  <MetricBox 
                    metric={t('HospitalizationsRealPanel.median')} 
                    enabled={data.statistics !== null}
              ...
              <Row>
                <Col xs={12} sm={6}>
          <MetricBox 
            metric={t('HospitalizationsRealPanel.min')} 
            enabled={data.statistics !== null}
            ...
            />
                ...
                <Col xs={12} sm={6}>
          <MetricBox 
            metric={t('HospitalizationsRealPanel.max')} 
            enabled={data.statistics !== null}
...
export default connect(mapStateToProps)(HospitalizationsRealPanel);
