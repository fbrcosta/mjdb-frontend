...
function DiagnosticsicdPanelIV(props) {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      ...
      await axios.get(process.env.REACT_APP_BACKEND_URL + '/mjdb/api/v1/statistics/diagnosticsicd/iv')
        .then(response => {
          console.log("cenas");
  ...
  return (
    <Box className={classes.root}>
      <GridContainer spacing={2}>
        ...
        <GridItem xs={12}>
          <CustomTable 
            title={t("DiagnosticsicdPanel.titleiv")}
            header={header}
...
export default connect(mapStateToProps)(DiagnosticsicdPanelIV);
