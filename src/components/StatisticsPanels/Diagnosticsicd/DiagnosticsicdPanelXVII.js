...
function DiagnosticsicdPanelXVII(props) {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      ...
      await axios.get(process.env.REACT_APP_BACKEND_URL + '/mjdb/api/v1/statistics/diagnosticsicd/xvii')
        .then(response => {
          console.log("cenas");
  ...
  return (
    <Box className={classes.root}>
      <GridContainer spacing={2}>
        ...
        <GridItem xs={12}>
          <CustomTable 
            title={t("DiagnosticsicdPanel.titlexvii")}
            header={header}
...
export default connect(mapStateToProps)(DiagnosticsicdPanelXVII);
