...
function DiagnosticsicdPanelXII(props) {
  ...
  React.useEffect(() => {
    const fetchData = async () => {
      ...
      await axios.get(process.env.REACT_APP_BACKEND_URL + '/mjdb/api/v1/statistics/diagnosticsicd/xii')
        .then(response => {
          console.log("cenas");
  ...
  return (
    <Box className={classes.root}>
      <GridContainer spacing={2}>
        ...
        <GridItem xs={12}>
          <CustomTable 
            title={t("DiagnosticsicdPanel.titlexii")}
            header={header}
...
export default connect(mapStateToProps)(DiagnosticsicdPanelXII);
